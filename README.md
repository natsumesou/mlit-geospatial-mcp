...
## 実行モード
このサーバーは以下の2つの起動モードに対応しています。

- `stdio` モード: Claude Desktop などからローカルMCPサーバーとして利用
- `http` モード: Streamable HTTP のMCPサーバーとして利用

デフォルトは `stdio` モードです。  
`MCP_TRANSPORT=http` を指定すると、HTTP サーバーとして起動します。
...
1. **Pythonをインストール**  
   ...
   * Pythonをインストールしてください。（バージョンは、3.10以上をインストールしてください。）  
   Pythonの公式ダウンロードページ（[https://www.python.org/downloads/windows/](https://www.python.org/downloads/windows/)）から、利用OSに合わせて3.10以上のインストーラーをダウンロードして、インストーラーを起動してください。
...
2. **Claude Desktopをダウンロード・インストール**
   * Claudeの公式ダウンロードページ([https://claude.com/download](https://claude.com/download))にアクセスし、インストーラーをダウンロードしてください。
   * ダウンロードしたファイルを実行するとインストールが開始されます。  
      画面の案内に従って「始める」をクリックし、アカウント認証を行うことでインストールが完了します。

3. **不動産情報ライブラリのAPI利用申請**
   * 不動産情報ライブラリの公式ページ([https://www.reinfolib.mlit.go.jp/api/request/](https://www.reinfolib.mlit.go.jp/api/request/))にアクセスし、API利用申請をしてください。   

4. **ソースコードをcloneする**
   * コマンドプロンプトを起動して、任意のディレクトリに移動する（例：C:\Users\username\）
   * 以下のコマンドでcloneする（mlit-geospatial-mcpディレクトリが自動的に作成されます）
   ```bash
...
社内向けのCloud Run検証や、HTTPクライアントからの接続確認をしたい場合は以下で起動できます。
...
export LIBRARY_API_KEY="your_api_key_here"
...
```bash
docker build -t mlit-geospatial-mcp .
docker run --rm -p 8080:8080 \
  -e LIBRARY_API_KEY="your_api_key_here" \
...
この構成は Cloud Run 上で単独実行できます。  
GeoJSON 保存先は Linux 系環境ではデフォルトで `/tmp/mlit-output` になります。

### 1. コンテナをビルドして Artifact Registry に push

```bash
gcloud builds submit --tag REGION-docker.pkg.dev/PROJECT_ID/REPOSITORY/mlit-geospatial-mcp
```

### 2. Cloud Run にデプロイ

```bash
gcloud run deploy mlit-geospatial-mcp \
  --image REGION-docker.pkg.dev/PROJECT_ID/REPOSITORY/mlit-geospatial-mcp \
  --region REGION \
  --platform managed \
  --no-allow-unauthenticated \
  --ingress internal-and-cloud-load-balancing \
  --set-env-vars MCP_TRANSPORT=http,LOG_LEVEL=WARNING
```

### 3. APIキーを環境変数で設定

```bash
gcloud run services update mlit-geospatial-mcp \
  --region REGION \
  --update-env-vars LIBRARY_API_KEY=your_api_key_here
```

運用時は `LIBRARY_API_KEY` を平文ではなく Secret Manager 経由で設定することを推奨します。

## Cloud Run 利用時の注意

- Cloud Run は永続ディスクを持たないため、`save_file=true` の保存先は一時領域です。
- 永続化したい場合は、`output_dir` を使うのではなく、別途 GCS 等への保存処理を追加してください。
- 既定では MCP エンドポイントは `/mcp` です。変更したい場合は `MCP_HTTP_PATH` を設定してください。
- Claude の custom connector には `https://<service-url>/mcp` を指定してください。
- リモート環境では DNS rebinding protection を既定で無効化しています。必要な場合のみ `MCP_DNS_REBINDING_PROTECTION=true` を設定し、あわせて `MCP_ALLOWED_HOSTS` と `MCP_ALLOWED_ORIGINS` をカンマ区切りで指定してください。
- SSL 検証は既定で有効です。必要な場合だけ `REQUESTS_VERIFY_SSL=false` を明示してください。
