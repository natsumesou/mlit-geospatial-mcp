# CHIRIKUKAN PLATFORM MCP Server
...
4. **仮想環境を作成 & 有効化**
   ...
   * 仮想環境の作成・有効化
   ...
   ```
   (.venv) C:\Users\username\chirikukan-mcp>の状態になれば成功です。
...
`claude_desktop_config.json`に下記を記載します。
   ...
   ```json
   {
      "mcpServers": {
         "chirikukan-mcp": {
            "command": "......./chirikukan-mcp/.venv/Scripts/python.exe",
            "args": [
               "....../chirikukan-mcp/src/server.py"
            ],
            "env": {
   ...
   * `command`と`args`は必ず、実際のパスに変更してください。  
   (例：C:/Users/username/chirikukan-mcp/.venv/Scripts/python.exe)
